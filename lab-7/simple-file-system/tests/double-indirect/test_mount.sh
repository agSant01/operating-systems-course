...
test-mount () {
    ...
    if diff -u <($TEST-input| ./bin/sfssh data/image.di.5 5 2> /dev/null) <($TEST-output) > test.log; then
    	echo "Success"
    else
    	echo "Failure"
	cat test.log
...
if diff -u <(bad-mount-input| ./bin/sfssh $SCRATCH/image.di.5 5 2> /dev/null) <(bad-mount-output) > $SCRATCH/test.log; then
    echo "Success"
else
    echo "Failure"
